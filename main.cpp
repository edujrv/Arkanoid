        ...
        switch (estado) {//Beginning If.
            ...
            case PODIO:
                ...
                if (Keyboard::isKeyPressed(Keyboard::Escape)) {
                    ...
                    soundBye.play();
                    sleep(seconds(1.5));
                    delete[] top; //Como hice un new hago un delete. Libero memoria
                    w.close();
                }
...
void compararTiempos(bestPlayer top[10], int userMin, int userSec, int userMs, std::string userName) { //Compara los tiempos del podio con los del jugador y si el jugador merece
    ...
    for (i = 0; i < 10; i++) {
        ...
        } else if (topSec[i] > userSec){        //Comparacion de los Segundos.
            ordenarTop(top, userMin, userSec, userMs, userName, i);
            break;

        } else if (topMs[i] > userMs){      //Comparacion de las milesimas de segundo.
            ordenarTop(top, userMin, userSec, userMs, userName, i);
            break;
        }
    }
